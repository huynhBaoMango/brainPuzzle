...
public class AdmobMediation : MonoBehaviour, AdsManager.AdsMediation
{
    ...
    public string AndroidAdmobAppId = "ca-app-pub-1015023790649631~2029185311";
    public string AndroidAdmobBannerAdsId = "ca-app-pub-1015023790649631/6897033450";
    public string AndroidAdmobFullscreenAdsId = "ca-app-pub-1015023790649631/3473079173";
    public string AndroidAdmobVideoAdsId = "ca-app-pub-1015023790649631/3145776118";
    ...
    {   
        ...
        if (s < ZenSDK.instance.GetConfigInt("fullscreenTime", 30) || ZenSDK.instance.GetConfigInt("levelDelayToPopAds", 4) < int.Parse(level))
        {
            PromoUI.Instance.OpenPromoUI();
            Debug.Log("Not enought wait time, show crosspromo");
            Debug.Log("Not enought level to pop ads");
            return;
        }
    ...
    void showFullscreen(string placement, string level){
        ...
        fullscreen_level = level.ToString();
    ...
    void RegisterEventHandlers(InterstitialAd ad)
    {
        ...
        ad.OnAdFullScreenContentOpened += () =>
        {
            ...
            ZenSDK.instance.TrackFullscreenStartShow(fullscreen_placement,fullscreen_level);
        };
        // Raised when the ad closed full screen content.
        ad.OnAdFullScreenContentClosed += () =>
        {
            Debug.Log("Interstitial ad full screen content closed.");
            ZenSDK.instance.TrackFullscreenEndShow(fullscreen_placement,fullscreen_level);
            initInterstitial();
        };
        ...
        ad.OnAdFullScreenContentFailed += (AdError error) =>
        {
            ...
            ZenSDK.instance.TrackFullscreenNotReady(fullscreen_placement,error.GetMessage());
    ...
    void RegisterEventHandlers(RewardedAd ad)
    {
        ...
        ad.OnAdFullScreenContentOpened += () =>
        {
            ...
            ZenSDK.instance.TrackRewardStartShow(video_placement,video_level);
        };
        // Raised when the ad closed full screen content.
        ad.OnAdFullScreenContentClosed += () =>
        {
            Debug.Log("Rewarded ad full screen content closed.");
            ZenSDK.instance.TrackRewardEndShow(video_placement,video_level);
            videoRewardReady = false;
            initVideoReward();
        };
        // Raised when the ad failed to open full screen content.
        ad.OnAdFullScreenContentFailed += (AdError error) =>
        {
            ...
            ZenSDK.instance.TrackRewardNotReady(video_placement,video_level,error.GetMessage());
            if (lastVideoRewardCallback != null)
            {
                lastVideoRewardCallback(false);
                lastVideoRewardCallback = null;
